...
if not client.is_user_authorized():
    ...
    client.sign_in(phone, input('Coloque seu código de segurança recebido: '))
...
g_index = input("Coloque somente um número: ")
target_group=groups[int(g_index)]

print('Colentando membros...')
all_participants = []
    ...
    try:
        for user in all_participants:
            ...
            if (accept):
                ...
                writer.writerow([i, username, user.id, user.access_hash, name, target_group.title, target_group.id, user.status.was_online])
                time.sleep(0.1)
